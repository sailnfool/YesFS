...
do
	...
	####################
	# Run b2sum to get the hash of each file in the driectory.
	# This should all be C Code.  Running this at least prevents
	# performing an "exec" of b2sum for each file in the directory.
	# 
	# All of the hashes and filenames are placed in a file that
	# we will read... "thislist"
	####################
	...
	do
		...
		####################	
		# Get the content hash ID of the file: CHID, and
		# create the path for the content hash file.
		# Also extract the filename.
		####################	
		chid=${hashline:0:128}
		p_chid=$(hashdirpath ${chid})
		manid=${p_chid}/${chid}.MANIFEST

		filename=${hashline:130}

		####################
		# Make sure we confirm we have taken the hash of
		# a filename.
		####################
		if [ ! -f "${filename}" ]
		then
		...
		then
			...
			####################
			# A prior manid means a prior MANIFEST, a safety
