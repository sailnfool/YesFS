...
then
	...
	if [[ -d "${tmpYesFSdir}" && -d "${YesFSdir}" ]] ; then
		...
		# copy
		########################################################
		rsync -az "${tmpYesFSdir}" "${YesFSdir}"
		rm -rf "${tmpYesFSdir}"
	fi
	if [[ ! -d "${YesFSdir}" ]] ; then
		export YesFSdir=${tmpYesFSdir}
	fi
	export YesFSdiretc=${YesFSdir}/etc

	if [[ ! -d "${YesFSdiretc}" ]] ; then
		mkdir -p "${YesFSdiretc}"
	fi

	export YesFSdirhash=${YesFSdir}/.hash
	export YesFSHASHES=${YesFSdirhash}
	export YesFSCHUNKLOG=${YesFSdir}/.chunklog

	export FAILEDTOHASH_string="Failed to hash \`"
	
	export FILECOUNT_lock="${YesFSdir}/FILECOUNT.lock"
	export FILECOUNT_file="${YesFSdir}/FILECOUNT.file"

	################################################################
	# hashoffset is the offset in the hashid array after the N
	# character Orthodox ID
	# -------
	# The following needs to be replaced by a function that
	# finds the ":" character at the beginning of an orthodox
	# crypto hash (OCH) representation.  The number of hash rows
	# and dir characters remain the same for now, but should be
	# replaced by dynamic variables instead of constant definitions.
	################################################################
	export YesFSdircharfile="${YesFSdiretc}/dirchars.txt"
	if [[ -r "${YesFSdircharfile}" ]] ; then
		YesFSdirchars=$(cat "${YesFSdircharfile}")
	else
		YesFSdirchars=4
		echo "${YesFSdirchars}" > "${YesFSdircharfile}"
	fi
	if [[ ! "${YesFSdirchars}" =~ ${bre_integer} ]] ; then
		echo "${FUNCNAME} YesFSdirchars must be an integer " \
			"but is ${YesFSdirchars}" >&2
		exit 1
	fi

	################################################################
	# Given that there are N YesFShashdirchars that are the prefix
	# of a hash value, it means that the number of hash rows is 
	# 0-FFFF (if N were 4)
	################################################################
	_x=${YesFSdirchars}
	_xx=""
	for ((i=0;i<${_x};i++));do _xx="${_xx}F";done
	YesFSflexhashrows=$(bfunc_hex2dec $_xx)
	((YesFSflexhashrows++)) # Count 0-FFFF
